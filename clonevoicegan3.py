...
def load_audio_and_preprocess(item, sr=22050, max_length=128):
    y, _ = librosa.load(example["audio"]["path"], sr=sr, backend='audioread')
    stft = librosa.stft(y)
...
all_spectrograms = [load_audio_and_preprocess(example) for example in dataset]
...
def load_audio_and_preprocess(example, sr=22050, max_length=128):  # argument is example
    y, _ = librosa.load(example["audio"]["path"], sr=sr, backend='audioread')
    stft = librosa.stft(y)
    spectrogram = librosa.amplitude_to_db(np.abs(stft), ref=np.max)
